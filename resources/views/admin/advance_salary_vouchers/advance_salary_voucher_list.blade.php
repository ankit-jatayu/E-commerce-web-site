...
?>
@extends('layouts.app')
@section('title','Adv. Salary Vouchers')
@section('content')
                            ...
                            <div class="row align-items-end">
                                ...
                                <div class="col-lg-4">
                                    <div class="page-header-breadcrumb">
                                        ...
                                        @if(isset($AdvanceSalaryVchModuleRights) && $AdvanceSalaryVchModuleRights->is_export==1)

                                    	<button type="button" class="btn btn-warning float-right ml-1 export">
                                            <i class="icofont icofont-file-spreadsheet"></i>
                                        </button>
                                        @endif
                                        @if(isset($AdvanceSalaryVchModuleRights) && $AdvanceSalaryVchModuleRights->is_create==1)

                                        <a href="{{route('advance.salary.voucher.add')}}" class="btn waves-effect waves-light btn-primary float-right"><i class="icofont icofont-plus"></i>Add New </a>
                                        @endif
                                    </div>
                                </div>
                            </div>
...
<script>
    $(document).ready(function () {
        ...
        var table  = $('#dt-ajax-array').DataTable({
            ...
            "scrollX":true,
            "ajax": {
                url: "{{route('advance.salary.voucher.paginate')}}",
                type: "POST",
                data: function(d) {
                    d._token = '{{csrf_token()}}';
        ...
        $('.export').click(function refreshData() {
            ...
            var url='{{route('advance.salary.voucher.export')}}?data='+JSON.stringify(param);
    ...
    function deleteRecord(id){
      if(confirm('are you sure want to delete this record?')){
        url='{{route('delete.advance.salary.voucher')}}';
    ...
    //  }
     ...
     function printVoucher(id){
       var url='{{route('print.advance.salary.voucher')}}?id='+id;
        //window.location.href=url;
