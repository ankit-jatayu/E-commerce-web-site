    ...
    <div class="modal-dialog modal-lg" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="row">
                <input type="hidden" id="account_type_id" value="{{$account_type_id}}">
    ...
    $(document).ready(function () {
        ...
        var table  = $('#dt-ajax-array').DataTable({
           ...
           // "pageLength": 100,
            ...
            "scrollX": true,
            "ajax": {
                url: "{{route('report.account.transaction.paginate')}}",
                type: "POST",
                data: function(d) {
                    d._token = '{{csrf_token()}}';
                ...
                if(data.length>0){
                  data.forEach((row,k)=>{
                    if(parseFloat(row['debit']) > 0){
                        total_debit +=(parseFloat(row['debit']));
                    }
                    if(parseFloat(row['credit']) > 0){
                        total_credit +=(parseFloat(row['credit']));
                    }
                    
                  })
                }
                total_balance = total_debit - total_credit;
                $(api.column(5).footer()).html(total_debit.toFixed(2));
                $(api.column(6).footer()).html(total_credit.toFixed(2));
                $(api.column(7).footer()).html(total_balance.toFixed(2));
            },
           });
        ...
        $('.export').click(function refreshData() {
            ...
            var url='{{route('report.account.transaction.export')}}?data='+JSON.stringify(param);
